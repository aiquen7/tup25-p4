...
function App() {
  ...
  useEffect(() => {
    ...
    const alumnosConFavoritos = alumnosParaMostrar.map(alumno => ({
      ...alumno,
      favorito: favoritos.has(alumno.legajo)
    }));

    const favoritosArray = alumnosConFavoritos.filter(a => a.favorito).sort(cmpNombre);
    const noFavoritosArray = alumnosConFavoritos.filter(a => !a.favorito).sort(cmpNombre);
  ...
  const favoritosArray = alumnosFiltrados.filter(a => a.favorito);
  const noFavoritosArray = alumnosFiltrados.filter(a => !a.favorito);
